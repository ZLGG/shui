 ...
 * @author OY
 */
public enum BbcTradeRightsStateEnum implements EnumMessage {
    ...
    BbcTradeRightsStateEnum(Integer code, String remark) {
        this.code = code;
        this.remark = remark;
    }
    ...
    public static String getRemarkByCode(Integer code) {
        ...
        for (BbcTradeRightsStateEnum tradeRightsStateEnum : BbcTradeRightsStateEnum.values()) {
            if (code.equals(tradeRightsStateEnum.getCode())) {
                return tradeRightsStateEnum.remark;
            }
        }
        return BbcTradeRightsStateEnum.待处理.remark;
    }
}
