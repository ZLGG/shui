package com.citydo.appraisal.utils.excel;
...
public class EasyExcelUtil {
    ...
    public static List<List<String>> readExcelWithStringList(InputStream inputStream, ExcelTypeEnum excelTypeEnum) {
        ...
        EasyExcel.read(inputStream,listener).excelType(excelTypeEnum).sheet().doRead();
    ...
    public static void writeExcelWithStringList(OutputStream outputStream, List<List<String>> data, WriteTable table, ExcelTypeEnum excelTypeEnum) {
        ...
        EasyExcel.write(outputStream).excelType(excelTypeEnum).sheet().doWrite(data);
    }
     ...
     * @return 返回 模型 的列表
     */
    public static <E> List<E> readExcelWithModel(InputStream inputStream, Class<?> clazz, ExcelTypeEnum excelTypeEnum,Integer headRowNumber) {
        // 解析每行结果在listener中处理
        ModelExcelListener<E> listener = new ModelExcelListener<E>();

        EasyExcel.read(inputStream, clazz, listener).excelType(excelTypeEnum).sheet().headRowNumber(Objects.isNull(headRowNumber) ? 1 : headRowNumber).doRead();
     ...
     * @param excelTypeEnum Excel的格式(XLS或XLSX)
     */
    public static void writeExcelWithModel(OutputStream outputStream, List<?> data,
                                           Class<?> clazz, ExcelTypeEnum excelTypeEnum)  {
