...
public interface MarketMerchantGiftMapper extends BaseMapper<MarketMerchantGift> {
    @Select("select goods.goods_id goods_id,goods.gift_id id,goods.shop_id shop_id,gift.gift_name gift_name,gift.scount_rule scount_rule  from gs_market_merchant_gift_goods goods LEFT JOIN gs_market_merchant_gift gift on goods.gift_id=gift.id  LEFT JOIN gs_goods_info gg ON gg.id=goods.goods_id where gg.goods_state=20 AND gift.state=20 AND gift.flag=0 And goods.flag=0 AND ${ew.sqlSegment}")
