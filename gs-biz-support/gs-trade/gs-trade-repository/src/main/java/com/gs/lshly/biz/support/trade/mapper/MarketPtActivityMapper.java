...
public interface MarketPtActivityMapper extends BaseMapper<MarketPtActivity> {
    ...
    @Select("SELECT * FROM gs_market_pt_activity WHERE flag=0  AND ${ew.sqlSegment} ")
    ...
    @Select("SELECT * FROM gs_market_pt_activity WHERE flag=0  AND ${ew.sqlSegment} ")
    IPage<BbcMarketActivityVO.activityListPageVO> activityListBbcPage(IPage<BbcMarketActivityVO.activityListPageVO> pager,@Param(Constants.WRAPPER) QueryWrapper<BbcMarketActivityQTO.QTO> query);
}
