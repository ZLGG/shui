...
public class BbcTradeServiceImpl implements IBbcTradeService {
     ...
     * 计算商城商品金额
     */
    private BbcTradeCommitGoodsCalculateDTO calculateAmountOfGood(List<BbcTradeBuildDTO.DTO.ProductData> productData, String shopId, String jwtUserId) {
        BbcTradeCommitGoodsCalculateDTO bbcTradeCommitGoodsCalculateDTO = new BbcTradeCommitGoodsCalculateDTO();
        // 店铺商品总金额
        BigDecimal shopProductAmount = BigDecimal.ZERO;
        // SKUID/购买数量
        List<CommonStockDTO.InnerChangeStockItem> subtractStockItems = new ArrayList<>();
        // 交易商品数据
        Set<BbcTradeGoodsDTO.ETO> tradeGoodsDTOSet = new HashSet<>();
        // 购物车ID
        List<String> cartIdList = new ArrayList<>();

        //店铺商品
        for (BbcTradeBuildDTO.DTO.ProductData singleProductData : productData) {
            log.info("productData:" + singleProductData.toString());
            log.info("productData.getCartId():" + singleProductData.getCartId());
            if ("gift".equals(singleProductData.getGoodsSkuId())) {
                continue;
            }
            //查询商品、判断上下架
            BbcGoodsInfoVO.InnerServiceVO innerServiceGoodsVO = bbcGoodsInfoRpc.innerServiceGoodsVO(singleProductData.getGoodsSkuId());
            ...
            //单品小计金额
            BigDecimal productPrice = innerServiceGoodsVO.getSalePrice().multiply(new BigDecimal(singleProductData.getQuantity()));
            ...
            innerChangeStockItem.setSkuId(singleProductData.getGoodsSkuId());
            innerChangeStockItem.setQuantity(singleProductData.getQuantity());
            subtractStockItems.add(innerChangeStockItem);
            //组装订单商品表信息
            BbcTradeGoodsDTO.ETO tradeGoodsDTO = fillTradeGoodsDTO(jwtUserId, 0, shopId, singleProductData.getQuantity(), innerServiceGoodsVO);
            tradeGoodsDTOSet.add(tradeGoodsDTO);
            //购物车ID
            if (singleProductData.getCartId() != null) {
                cartIdList.add(singleProductData.getCartId());
            }
        }
        bbcTradeCommitGoodsCalculateDTO.setShopProductAmount(shopProductAmount);
        bbcTradeCommitGoodsCalculateDTO.setSubtractStockItems(subtractStockItems);
        bbcTradeCommitGoodsCalculateDTO.setTradeGoodsDTOSet(tradeGoodsDTOSet);
        bbcTradeCommitGoodsCalculateDTO.setCartIdList(cartIdList);
        return bbcTradeCommitGoodsCalculateDTO;
    }

    /**
     * 计算积分商城商品金额
     */
    private BbcTradeCommitGoodsCalculateDTO calculateAmountOfPointGood(List<BbcTradeBuildDTO.DTO.ProductData> productData, String shopId, String jwtUserId) {
        BbcTradeCommitGoodsCalculateDTO bbcTradeCommitGoodsCalculateDTO = new BbcTradeCommitGoodsCalculateDTO();
        // 积分总金额
        BigDecimal shopProductPointAmount = BigDecimal.ZERO;
        // SKUID/购买数量
        List<CommonStockDTO.InnerChangeStockItem> subtractStockItems = new ArrayList<>();
        // 交易商品数据
        Set<BbcTradeGoodsDTO.ETO> tradeGoodsDTOSet = new HashSet<>();
        ...
        //店铺商品
        for (BbcTradeBuildDTO.DTO.ProductData singleProductData : productData) {
            log.info("singleProductData:" + singleProductData.toString());
            log.info("singleProductData.getCartId():" + singleProductData.getCartId());
            if ("gift".equals(singleProductData.getGoodsSkuId())) {
                continue;
            }
            //查询商品、判断上下架
...
//            innerServiceGoodsVO.setPointPrice(new BigDecimal(5));
            ...
            if (innerServiceGoodsVO.getIsInMemberGift()) {
                // 如果是IN会员礼品，判断是否是IN会员
                if (!"1".equals(isInUser)) {
                    throw new BusinessException("存在IN会员专属商品，请开通IN会员后重试");
                }
                pointPrice = innerServiceGoodsVO.getInMemberPointPrice().multiply(new BigDecimal(singleProductData.getQuantity()));
            } else {
                pointPrice = innerServiceGoodsVO.getPointPrice().multiply(new BigDecimal(singleProductData.getQuantity()));
            }
            ...
            subtractStockItems.add(innerChangeStockItem);
            //组装订单商品表信息
            BbcTradeGoodsDTO.ETO tradeGoodsDTO = fillTradeGoodsDTO(jwtUserId, isInUser, shopId, singleProductData.getQuantity(), innerServiceGoodsVO);
            tradeGoodsDTOSet.add(tradeGoodsDTO);
            //购物车ID
            if (singleProductData.getCartId() != null) {
                cartIdList.add(singleProductData.getCartId());
        ...
        bbcTradeCommitGoodsCalculateDTO.setShopProductPointAmount(shopProductPointAmount);
        bbcTradeCommitGoodsCalculateDTO.setSubtractStockItems(subtractStockItems);
        bbcTradeCommitGoodsCalculateDTO.setTradeGoodsDTOSet(tradeGoodsDTOSet);
        bbcTradeCommitGoodsCalculateDTO.setCartIdList(cartIdList);
        return bbcTradeCommitGoodsCalculateDTO;
    ...
    private void saveTradePay(Trade trade) {
        ...
        if (tradePayTypeEnum.getMixedPayment()) {
            ...
            tradePayRepository.save(tradePay);
            tradePay.setPayType(tradePayTypeEnum.getSubType().getCode());
     ...
     * @return
     */
    private Trade saveTrade(BbcTradeBuildDTO.DTO dto, BbcStockAddressVO.ListVO addressVO) {
     ...
     * @return
     */
    private BbcTradeGoodsDTO.ETO fillTradeGoodsDTO(String userId, Integer isInUser, String shopId, Integer quantity, BbcGoodsInfoVO.InnerServiceVO innerServiceGoodsVO) {
        ...
        tradeGoodsDTO.setCouponType(TradeCouponTypeEnum.普通.getCode());
        // 获取价格
        if (innerServiceGoodsVO.getIsPointGood()) {
            BigDecimal pointPrice;
            if (innerServiceGoodsVO.getIsInMemberGift()) {
                ...
                pointPrice = innerServiceGoodsVO.getInMemberPointPrice().multiply(new BigDecimal(quantity));
                tradeGoodsDTO.setCouponType(TradeCouponTypeEnum.IN会员.getCode());
            } else {
                pointPrice = innerServiceGoodsVO.getPointPrice().multiply(new BigDecimal(quantity));
            }
            tradeGoodsDTO.setPointPrice(pointPrice);
        } else {
...
//                }
    ...
    public PageData<BbcTradeListVO.tradeVO> tradeListPageData(BbcTradeQTO.TradeList qto) {
        ...
        IPage<BbcTradeListVO.tradeVO> page = MybatisPlusUtil.pager(qto);

        tradeRepository.selectTradeListPage(page, wrapper);

        List<BbcTradeListVO.tradeVO> voList = new ArrayList<>();
        for (BbcTradeListVO.tradeVO tradeVO : page.getRecords()) {
            //查询店铺信息
            BbcShopQTO.InnerShopQTO innerShopQTO = new BbcShopQTO.InnerShopQTO();
    ...
    public ResponseData<Void> orderConfirmReceipt(BbcTradeDTO.IdDTO dto) {
        ...
        dto1.setNumber(trade.getId()).setPlatformId("dpos1009").setShop("dpos1009");
    ...
    private PosTradeODeliverOrderRequestDTO.DTO getPOSDTO(Trade trade, TradePay tradePay) {
        ...
        if (ObjectUtils.isNotEmpty(list)) {
            ...
            for (TradeGoods goods : list) {
                ...
                rsThinSku2.setId(goods.getGoodsId());
        ...
        oOnlineOrderPayment.setPayTime(Date.from(tradePay.getCdate().atZone(ZoneId.systemDefault()).toInstant())).
                setPayMethodCode(tradePay.getTradeCode()).
                setPayMethodName(ObjectUtils.isNotEmpty(tradePay.getPayState()) ? EnumUtil.getText(tradePay.getPayState(), TradePayStateEnum.class) : null).
...
//        cancelTradeReturnStock(trade.getId());
    ...
    private TradeRights inset(Trade trade, BbcTradeCancelDTO.CancelDTO dto) {
        ...
        tradeRights.setTradeId(trade.getId()).
                ...
                setRightsType(TradeRightsTypeEnum.取消订单.getCode()).
    ...
    private void fillTradeCancel(BbcTradeCancelDTO.CancelDTO dto, TradeCancel tradeCancel, Trade trade, String
            tradePayId, Integer cancelState, Integer cancelRefundState) {
...
//        request.setTradeType("WECHATAPP");
     ...
     */
    private QRCodePaymentCommitResponse doPay(String openid, BossClient client, EntOffLinePaymentResponse
            entOffLinePaymentResponse) throws Exception {
