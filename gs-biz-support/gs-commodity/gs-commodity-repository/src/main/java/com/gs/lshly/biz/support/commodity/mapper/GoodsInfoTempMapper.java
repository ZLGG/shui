...
public interface GoodsInfoTempMapper extends BaseMapper<GoodsInfoTemp> {
    ...
    @Select(
            "SELECT DISTINCT gs.id,gs.merchant_id,gs.shop_id,gs.brand_id,gs.spec_info_id,gs.attribute_info_id," +
                    ...
                    "gs.is_show_old_price,gs.goods_price_unit,gs.use_platform,gs.publish_time,gs.cdate as commit_time,gs.udate,gs.apply_type  from  gs_goods_info_temp gs \n" +
    ...
    @Select(
            "SELECT DISTINCT gs.id,gs.merchant_id,gs.shop_id,gs.brand_id,gs.spec_info_id,gs.attribute_info_id," +
                    ...
                    "gs.is_show_old_price,gs.goods_price_unit,gs.use_platform,gs.publish_time,gs.cdate,gs.udate,gs.apply_type from  gs_goods_info_temp gs \n" +
                    "WHERE gs.flag =0 AND ${ew.sqlSegment} "
    )
